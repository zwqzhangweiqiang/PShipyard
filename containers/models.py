...
class Host(models.Model):
    ...
    def _get_client(self):
        ...
        return client.Client(base_url=url, version="1.17")
    ...
    def get_images(self, show_all=False):
        ...
        if images is None:
            try:
                old_images = c.images(all=show_all)
                ...
                for x in old_images:
                    ...
                    for tag in tags:
                        t = x.copy()
                        tag_fileds = tag.split(":")
                        t['Repository'] = ':'.join(tag_fileds[:-1])
                        t['Tag'] = tag_fileds[-1]
                        images.append(t)
                #images = [x for x in images if x.get('Repository')]
                print images
...
class Container(models.Model):
    ...
    def get_ports(self):
        ...
        if self.host.version < '0.6.5':
            ...
            if not network_settings.get('Ports', {}):
                return ports
